...
class _HomePage1State extends State<HomePage1> {
  ...
  List<dynamic> listOfPopularMovies = [];
  List<dynamic> listOfUpcomingMovies = [];
  String? name;
  ...
  Future<void> getMovies() async {
    String url = 'http://10.10.10.139/api/get_movie';
    Dio dio = Dio();
    try {
      ...
      log("${response.data}");
      setState(() {
        listOfMovies = response.data['movies'];
        listOfPopularMovies = response.data['popularMovies'];
        listOfUpcomingMovies = response.data['upcomingMovies'];
      });
    } catch (e) {
                        ...
                        itemBuilder: (context, index, realIdx) {
                          return InkWell(
                            ...
                            child: Stack(
                              children: [
                                Image.network(
                                  carouselMovieData[index]['imageUrl']
                                      .toString(),
                                  fit: BoxFit.cover,
                                  width: 1000,
                                ),
